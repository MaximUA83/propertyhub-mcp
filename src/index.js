...
const tools = [
  ...
  { name: 'list_invoices', description: 'Отримати список рахунків. Фільтри: month (0-11), year, status (created/signing/signed/sent/paid), property_id, unpaid_only.', inputSchema: { type: 'object', properties: { month: { type: 'number', description: '0-11 (січень=0, лютий=1, березень=2, квітень=3, травень=4, червень=5, липень=6, серпень=7, вересень=8, жовтень=9, листопад=10, грудень=11)' }, year: { type: 'number' }, status: { type: 'string' }, property_id: { type: 'number' }, unpaid_only: { type: 'boolean' } } } },
  ...
  { name: 'list_utilities', description: 'Отримати список записів про комунальні платежі постачальникам.', inputSchema: { type: 'object', properties: { month: { type: 'number', description: '0-11 (січень=0, лютий=1, березень=2, квітень=3, травень=4, червень=5, липень=6, серпень=7, вересень=8, жовтень=9, листопад=10, грудень=11)' }, year: { type: 'number' }, property_id: { type: 'number' } } } },
  { name: 'add_utility', description: 'Додати комунальний платіж постачальнику. Типи: electricity, water, heating, hcs.', inputSchema: { type: 'object', properties: { property_id: { type: 'number' }, type: { type: 'string' }, month: { type: 'number', description: '0-11 (січень=0, лютий=1, березень=2, квітень=3, травень=4, червень=5, липень=6, серпень=7, вересень=8, жовтень=9, листопад=10, грудень=11)' }, year: { type: 'number' }, provider_amount: { type: 'number' }, note: { type: 'string' } }, required: ['property_id', 'type', 'month', 'year', 'provider_amount'] } },
  ...
  { name: 'create_invoice', description: 'Створити рахунок. Режими: full, rent_only, utilities_only.', inputSchema: { type: 'object', properties: { property_id: { type: 'number' }, month: { type: 'number', description: '0-11 (січень=0, лютий=1, березень=2, квітень=3, травень=4, червень=5, липень=6, серпень=7, вересень=8, жовтень=9, листопад=10, грудень=11)' }, year: { type: 'number' }, mode: { type: 'string' }, exchange_rate_usd: { type: 'number' }, exchange_rate_eur: { type: 'number' } }, required: ['property_id', 'month', 'year'] } },
  { name: 'create_invoices_bulk', description: 'Створити рахунки всім орендарям або обраним.', inputSchema: { type: 'object', properties: { month: { type: 'number', description: '0-11 (січень=0, лютий=1, березень=2, квітень=3, травень=4, червень=5, липень=6, серпень=7, вересень=8, жовтень=9, листопад=10, грудень=11)' }, year: { type: 'number' }, mode: { type: 'string' }, property_ids: { type: 'array', items: { type: 'number' } }, exchange_rate_usd: { type: 'number' }, exchange_rate_eur: { type: 'number' } }, required: ['month', 'year'] } },
  ...
  { name: 'get_income_report', description: 'Звіт про отримані доходи (оплати) за період. Параметри: year (обов\'язково), month (0-11 опціонально, якщо не вказано — весь рік), group_by (day/month/property — як групувати). Повертає суми у гривнях з групуванням.', inputSchema: { type: 'object', properties: { year: { type: 'number' }, month: { type: 'number', description: '0-11 (січень=0, лютий=1, березень=2, квітень=3, травень=4, червень=5, липень=6, серпень=7, вересень=8, жовтень=9, листопад=10, грудень=11)' }, group_by: { type: 'string', description: 'day, month, property (default: month)' } }, required: ['year'] } },
  ...
  { name: 'get_financial_summary', description: 'Загальний фінансовий підсумок: нараховано/сплачено/заборговано за період. Параметри: year, month (опціонально). Показує по кожному приміщенню і загалом.', inputSchema: { type: 'object', properties: { year: { type: 'number' }, month: { type: 'number', description: '0-11 (січень=0, лютий=1, березень=2, квітень=3, травень=4, червень=5, липень=6, серпень=7, вересень=8, жовтень=9, листопад=10, грудень=11)' } }, required: ['year'] } },
  ...
  { name: 'add_tax_payment', description: 'Записати факт сплати податку. Типи: single_tax (єдиний), military_tax (військовий), esv, other. Приклад: "запиши що 15 квітня заплатили єдиний за Q1 — 9000 грн, платіжка №123".', inputSchema: { type: 'object', properties: { year: { type: 'number' }, quarter: { type: 'number' }, month: { type: 'number', description: '0-11 (січень=0, лютий=1, березень=2, квітень=3, травень=4, червень=5, липень=6, серпень=7, вересень=8, жовтень=9, листопад=10, грудень=11)' }, type: { type: 'string', description: 'single_tax, military_tax, esv, other' }, amount: { type: 'number' }, paid_date: { type: 'string', description: 'YYYY-MM-DD' }, note: { type: 'string' } }, required: ['year', 'type', 'amount'] } },
...
async function handleTool(name, args) {
  switch (name) {
    ...
    case 'send_invoice_email': {
      ...
      const count = r.recipientsCount || 1;
      const msg = r.ok
        ? (count > 1 ? `Надіслано на ${count} адрес: ${r.sentTo}` : `Надіслано на ${r.sentTo}`)
        : 'Помилка';
      return {
        success: r.ok,
        sent_to: r.sentTo,
